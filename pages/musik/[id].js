      ...
      <div style={{ minHeight: '80vh', backgroundColor: 'black', padding: '40px 50px', color: 'white', paddingBottom: '120px' }}>
        ...
        <div style={{ maxWidth: '600px', margin: '0 auto', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '40px' }}>
            <button disabled={!track.audioUrl || (currentTrack && currentTrack.id !== track.id)} onClick={() => seek(Math.max(0, displayCurrentTime - 10))} style={{ background: 'transparent', border: 'none', cursor: track.audioUrl ? 'pointer' : 'not-allowed', opacity: track.audioUrl ? 1 : 0.5 }}>
            ...
            <button disabled={!track.audioUrl || (currentTrack && currentTrack.id !== track.id)} onClick={() => seek(Math.min(displayDuration, displayCurrentTime + 10))} style={{ background: 'transparent', border: 'none', cursor: track.audioUrl ? 'pointer' : 'not-allowed', opacity: track.audioUrl ? 1 : 0.5 }}>
              <svg width="32" height="32" viewBox="0 0 24 24" fill="none">
